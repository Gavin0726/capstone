...
def create_app(test_config=None):
    ...
    def retrieve_movies(payload):
        ...
        for movie in movies:
            ...
            actors = []
            results = Assign.query.filter(Assign.movie_id == movie["id"]).all()
            if results:
                for result in results:
                    if result.movie_id == movie["id"]:
                        actor = {
    ...
    def retrieve_actors(payload):
        ...
        for actor in actors:
            ...
            movies = []
            results = Assign.query.filter(Assign.actor_id == actor["id"]).all()

            if results:
                for result in results:
                    if result.actor_id == actor["id"]:
                        movie = {
                          "movie_id": result.movie_id,
                          "movie_name": result.movie.title
                        }
                        movies.append(movie)
                actor["movies"] = []
                actor["movies"].append(movies)
